...
public class MessageRow {
	...
	public MessageRow(ResultSet rs, boolean validateNames) throws SQLException {
        if(validateNames || jIRCTools.isValidColumn(rs, "fk_ChannelID"))
            this.fk_ChannelID = rs.getInt("fk_ChannelID");
        
        if(validateNames || jIRCTools.isValidColumn(rs, "pk_MessageID"))
            this.pk_MessageID = rs.getInt("pk_MessageID");
        
        if(validateNames || jIRCTools.isValidColumn(rs, "vcUsername"))
            this.vcUsername = rs.getString("vcUsername");
        
        if(validateNames || jIRCTools.isValidColumn(rs, "vcMessage"))
            this.vcMessage = rs.getString("vcMessage");
        
        if(validateNames || jIRCTools.isValidColumn(rs, "vcMsgType"))
            this.msgType = eMsgTypes.valueOf(rs.getString("vcMsgType"));
        
        if(validateNames || jIRCTools.isValidColumn(rs, "tsMsgTime"))
